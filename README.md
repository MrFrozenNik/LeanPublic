...
---
Существующие проблемы: зашитый .env в докер образе, невозможность опустить и поднять контейнер из-за проблем с entrypoint.sh. TODO - исправить
---
...
# 6. Открыть в браузере (подставьте свой домен, запись у которого в DNS указывает на IP сервера)
...
# (подставьте свой домен, запись у которого в DNS указывает на IP сервера)
